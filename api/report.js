export default async function handler(req, res) {
    ...
    (async () => {
      ...
      for (const mirror of MIRRORS) {
        try {
          ...
          if (text.startsWith('{')) { parsed = JSON.parse(text); break; }
          console.log('Mirror', mirror, 'returned non-JSON, trying next...');
        } catch(e) { console.log('Mirror', mirror, 'failed:', e.message); }
      }
      if (!parsed) throw new Error('All Overpass mirrors failed');
      return parsed;
    })(),
    // SOLAR
    (async () => {
      // LV03 Koordinaten berechnen fuer geo.admin.ch
    function wgsToLv03(lat, lon) {
      const lat_ = (lat * 3600 - 169028.66) / 10000;
      const lon_ = (lon * 3600 - 26782.5) / 10000;
      const x = 200147.07 + 308807.95*lat_ + 3745.25*lon_*lon_ + 76.63*lat_*lat_ - 194.56*lon_*lon_*lat_ + 119.79*lat_*lat_*lat_;
      const y = 600072.37 + 211455.93*lon_ - 10938.51*lon_*lat_ - 0.36*lon_*lat_*lat_ - 44.54*lon_*lon_*lon_;
      return { x: Math.round(x), y: Math.round(y) };
    }
    const lv03 = wgsToLv03(lat, lon);
    const lv03Extent = `${lv03.y-500},${lv03.x-500},${lv03.y+500},${lv03.x+500}`;
        const solarUrl = `https://api3.geo.admin.ch/rest/services/api/MapServer/identify?geometry=${lv03.y},${lv03.x}&geometryType=esriGeometryPoint&layers=all:ch.bfe.solarenergie-eignung-fassaden,all:ch.bafu.laerm-strassenlaerm_tag&mapExtent=${lv03Extent}&imageDisplay=100,100,96&tolerance=100&returnGeometry=false&sr=21781`;
  ...
  // Process Solar + Noise result (combined identify call)
  if (solarResult.status === 'fulfilled') {
    try {
      const results = solarResult.value.results || [];
      console.log('GEO.ADMIN identify results:', results.length, results.map(r=>r.layerBodId));
      results.forEach(r => {
        const props = r.attributes || {};
        if (r.layerBodId === 'ch.bfe.solarenergie-eignung-fassaden') {
          const val = props.gstrahlung || props.stromertrag || props.klasse || props.eignung || props.value || null;
          if (val) solarKwh = parseFloat(val);
        }
        if (r.layerBodId === 'ch.bafu.laerm-strassenlaerm_tag') {
          // BAFU noise layer: field is typically 'db_tag' or 'laeq' or numeric value
          const db = props.db_tag || props.laeq || props.klasse || props.value || props.lr_tag || null;
          if (db !== null && db !== undefined) {
            noiseDay = parseFloat(db);
            noiseSource = 'BAFU Lärmdaten';
            console.log('BAFU NOISE:', noiseDay, 'props:', JSON.stringify(props));
          }
        }
      });
    } catch(e) { console.log('GEO.ADMIN process error:', e.message); }
  } else { console.log('GEO.ADMIN error:', solarResult.reason?.message); }
