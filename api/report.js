export default async function handler(req, res) {
  ...
  const { address, rooms, area, price, type, year, floor, outdoor, condition, insertText, parsedInsert } = req.body;
  ...
  if (solarResult.status === 'fulfilled' && solarResult.value) {
    try {
      ...
      // BAFU Noise via WMS GetFeatureInfo
      if (noisePixels && lv95E && lv95N) {
        try {
          ...
          if (isPng) {
            const noiseDelta = 100;
            const noiseBbox2 = `${lv95E-noiseDelta},${lv95N-noiseDelta},${lv95E+noiseDelta},${lv95N+noiseDelta}`;
            const noiseInfoUrl = `https://wms.geo.admin.ch/?SERVICE=WMS&VERSION=1.3.0&REQUEST=GetFeatureInfo&LAYERS=ch.bafu.laerm-strassenlaerm_tag&QUERY_LAYERS=ch.bafu.laerm-strassenlaerm_tag&CRS=EPSG:2056&BBOX=${noiseBbox2}&WIDTH=11&HEIGHT=11&I=5&J=5&INFO_FORMAT=text/plain&STYLES=`;
            const noiseInfoRes = await fetch(noiseInfoUrl, { signal: AbortSignal.timeout(5000) });
            const noiseInfoText = await noiseInfoRes.text();
            console.log('BAFU NOISE GetFeatureInfo response:', noiseInfoText.substring(0,300));
            const dbMatch = noiseInfoText.match(/([3-9][0-9](?:\.[0-9])?)\s*(?:dB|Lr|db)/i);
            if (dbMatch) {
              noiseDay = parseFloat(dbMatch[1]);
              noiseSource = 'BAFU Strassenlaerm';
              console.log('BAFU NOISE dB:', noiseDay);
            } else if (noiseInfoText.length < 50 || noiseInfoText.includes('no data') || noiseInfoText.includes('nodata')) {
              noiseDay = 35;
              noiseSource = 'BAFU (kein Strassenlaerm erfasst - ruhige Lage)';
              console.log('BAFU NOISE: no data = quiet location');
            } else {
              console.log('BAFU NOISE: could not parse dB from:', noiseInfoText.substring(0,100));
            }
          }
        } catch(noiseErr) {
          console.log('BAFU NOISE error:', noiseErr.message);
        }
      }
  ...
  const TAXES = {
    ...
    'lausanne':79,'pully':70,'lutry':68,
    'luzern':107,'lucerne':107,
    ...
    'schaffhausen':113,'winterthur':122,'frauenfeld':106,
