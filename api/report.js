export default async function handler(req, res) {
  ...
  const [overpassResult, solarResult] = await Promise.allSettled([
    ...
    const lv95Extent = `${lv95E-500},${lv95N-500},${lv95E+500},${lv95N+500}`;
        const solarUrl = `https://api3.geo.admin.ch/rest/services/all/MapServer/identify?geometry=${lv95E},${lv95N}&geometryType=esriGeometryPoint&layers=all:ch.bfe.solarenergie-eignung-daecher&mapExtent=${lv95Extent}&imageDisplay=100,100,96&tolerance=50&returnGeometry=false&sr=2056`;
  ...
  if (solarResult.status === 'fulfilled') {
    try {
      ...
      if (results.length) {
        ...
        console.log('SOLAR props:', JSON.stringify(props).substring(0,200));
        // Fields: gstrahlung (kWh/m2/yr), stromertrag, klasse (1-5)
        const val = props.gstrahlung || props.stromertrag || props.klasse || props.eignung || null;
