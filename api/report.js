export default async function handler(req, res) {
  ...
  const [overpassResult, solarResult] = await Promise.allSettled([
    ...
    (async () => {
      ...
      // Use POST to Overpass mirrors (more reliable from server environments)
      const MIRRORS = [
      ...
      for (let mi = 0; mi < MIRRORS.length; mi++) {
        ...
        const timeout = mi === 0 ? 5000 : 12000; // first mirror gets 5s, rest 12s
        try {
          ...
          console.log('Mirror', mirror, 'returned non-JSON:', text.substring(0,80));
    ...
    // SOLAR
    (async () => {
      // WGS84 -> LV95 (sr=2056) fuer geo.admin.ch
  ...
  // Process Solar result (Sonnendach.ch daecher layer)
  if (solarResult.status === 'fulfilled') {
    try {
      const results = solarResult.value.results || [];
      console.log('SOLAR results:', results.length, results.map(r=>r.layerBodId));
      if (results.length) {
        const props = results[0].attributes || results[0].properties || {};
        console.log('SOLAR props:', JSON.stringify(props).substring(0,400));
        // Fields: gstrahlung (kWh/m2/yr), stromertrag, klasse (1-5)
        // klasse 1-5: 1=wenig, 5=sehr gut. Map to kWh/m2/yr for scoring
        const klasse = props.klasse;
        const gstrahlung = props.gstrahlung;
        // Use gstrahlung (kWh/yr total roof) or derive from klasse
        const val = klasse ? [null,600,750,900,1050,1200][klasse] : (gstrahlung ? gstrahlung/10 : null);
        console.log('SOLAR field picked:', {klasse, gstrahlung, val});
