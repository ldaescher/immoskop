...
- Zimmeranzahl ist EXAKT ${rooms} Zimmer. Nenne NIEMALS eine andere Zahl. Wenn du im Inseratstext eine andere Zahl siehst, zitiere sie explizit als Widerspruch — erfinde aber keine dritte Zahl.
- Baujahr laut Formular: ${year}. Wenn der Inseratstext ein anderes Baujahr nennt, zitiere beide. Erfinde kein drittes Baujahr.
- Kommentiere NUR was in den Daten oder im Inseratstext steht. Verweise nicht auf Dinge die nicht vorhanden sind.
- "Nicht verfügbar" ist besser als eine erfundene Angabe.
...
${estvResult ? (() => {
    const taxLabel = estvNatPct <= 5 ? 'ausgesprochen tief — gehört zu den 5% günstigsten Gemeinden der Schweiz ★★★★★' :
                    estvNatPct <= 20 ? 'tief — günstig im nationalen Vergleich ★★★★' :
                    estvNatPct <= 45 ? 'moderat — leicht unter Schweizer Durchschnitt ★★★' :
                    estvNatPct <= 65 ? 'durchschnittlich ★★★' :
                    estvNatPct <= 80 ? 'leicht überdurchschnittlich ★★' : 'hoch ★';
    const ktLabel = estvKtPct <= 15 ? `eine der ${estvKtPct <= 5 ? 'günstigsten' : 'günstigen'} Gemeinden im Kanton ${estvKanton}` :
                    estvKtPct >= 85 ? `eine der teuersten Gemeinden im Kanton ${estvKanton}` :
                    `im mittleren Bereich des Kantons ${estvKanton}`;
    return `
${s.taxSection}
Kombinierter Steuerfuss (Kanton + Gemeinde): ${estvTotal}% der einfachen Steuer — ${taxLabel}.
Kantonsvergleich: ${ktLabel} (${estvKtPct}. Perzentile).
Nationaler Vergleich: günstiger als ${100 - estvNatPct}% aller Schweizer Gemeinden (${estvNatPct}. Perzentile).
Hinweis: Der kombinierte Steuerfuss ist kantonsübergreifend vergleichbar. Quelle: ESTV 2025.`;
  })() : ''}
