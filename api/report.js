export default async function handler(req, res) {
  ...
  const [placesResult, solarResult] = await Promise.allSettled([
    // GEOAPIFY Places (ÖV + Amenities + Strassen)
    (async () => {
      ...
      // Fetch amenities
      const amenUrl = `https://api.geoapify.com/v2/places?categories=education.school,education.kindergarten,commercial.supermarket,commercial.food_and_drink.supermarket,catering.restaurant,catering.cafe,natural.park,leisure.park,healthcare.pharmacy&filter=circle:${lon},${lat},800&limit=100&apiKey=${GEOAPIFY_KEY}`;
      // Fetch roads for noise estimate
      // Road noise via Geoapify routing (unused for now)

      console.log('GEOAPIFY amenUrl:', amenUrl.replace(GEOAPIFY_KEY, 'KEY'));
      const [oevRes, amenRes] = await Promise.all([
        fetch(oevUrl, { signal: AbortSignal.timeout(8000) }).then(r => r.json()),
        fetch(amenUrl, { signal: AbortSignal.timeout(8000) }).then(r => r.json())
      ]);
      return { oev: oevRes, amen: amenRes };
    })(),
    // SOLAR (geo.admin.ch LV95)
    (async () => {
  ...
  if (placesResult.status === 'fulfilled' && placesResult.value) {
    try {
      const { oev: oevData, amen: amenData } = placesResult.value;
      const oevFeatures = oevData?.features || [];
      const amenFeatures = amenData?.features || [];
      ...
      if (oevFeatures.length > 0) {
        ...
        oevFeatures.forEach(f => {
          ...
          if (dist < minDist) { minDist=dist; oevName=f.properties?.name||f.properties?.datasource?.raw?.name||'Haltestelle'; }
        });
