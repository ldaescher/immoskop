export default async function handler(req, res) {
  ...
  const [placesResult, solarResult] = await Promise.allSettled([
    ...
    (async () => {
      ...
      // Fetch ÖV stops
      const oevUrl = `https://api.geoapify.com/v2/places?categories=public_transport.train,public_transport.tram,public_transport.bus&filter=circle:${lon},${lat},800&limit=50&apiKey=${GEOAPIFY_KEY}`;
      // Fetch amenities
      const amenUrl = `https://api.geoapify.com/v2/places?categories=education.school,education.kindergarten,commercial.supermarket,catering.restaurant,catering.cafe,leisure.park,healthcare.pharmacy&filter=circle:${lon},${lat},800&limit=50&apiKey=${GEOAPIFY_KEY}`;
      // Fetch roads for noise estimate
      const roadUrl = `https://api.geoapify.com/v1/routing?waypoints=${lat},${lon}&mode=drive&apiKey=${GEOAPIFY_KEY}`;
    ...
    try {
      ...
      if (oevFeatures.length > 0) {
        ...
        oevFeatures.forEach(f => {
          ...
          if (dist < minDist) { minDist=dist; oevName=f.properties?.name||f.properties?.street||'Haltestelle'; }
        });
      ...
      amenFeatures.forEach(f => {
        ...
        if (cats.some(c=>c.includes('education')) && groups.Schulen.length<3) groups.Schulen.push(`${name} ${dist}m`);
        if (cats.some(c=>c.includes('supermarket')) && groups.Einkauf.length<3) groups.Einkauf.push(`${name} ${dist}m`);
        if (cats.some(c=>c.includes('catering')) && groups.Gastro.length<3) groups.Gastro.push(`${name} ${dist}m`);
        if (cats.some(c=>c.includes('park')) && groups.Parks.length<2) groups.Parks.push(`${name} ${dist}m`);
        if (cats.some(c=>c.includes('healthcare')) && groups.Gesundheit.length<2) groups.Gesundheit.push(`${name} ${dist}m`);
      });
