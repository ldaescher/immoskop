export default async function handler(req, res) {
  ...
  'hüntwangen':108,
  'hüttikon':113,
  'illnau effretikon':113,
  'kappel':102,
  'kappel aa':102,
  'kilchberg':72,
  'kleinandelfingen':118,
  'kloten':100,
  'knonau':116,
  'küsnacht':73,
  'langnau':106,
  ...
  function lookupTax(communeLower, plz) {
    const plzStr = String(plz || '');
    // Normalize: remove dots, umlauts-alt forms
    const norm = communeLower.replace(/\./g,'').replace(/-/g,' ').replace(/\s+/g,' ').trim();
    // Direct match
    if (TAX_ZH[norm] !== undefined) return TAX_ZH[norm];
    if (TAX_ZH[communeLower] !== undefined) return TAX_ZH[communeLower];
    // Partial match (commune name without suffix like "a.A.")
    const short = norm.split(' a')[0].trim();
    if (short.length > 3 && TAX_ZH[short] !== undefined) return TAX_ZH[short];
    // Fuzzy: find any key that contains communeLower's first word
    const firstWord = norm.split(' ')[0];
    if (firstWord.length > 4) {
      const match = Object.entries(TAX_ZH).find(([k]) => k.startsWith(firstWord));
      if (match) return match[1];
    }
    // Other cantons fallback
    const OTHER_TAX = {'bern':159,'basel':90,'luzern':128,'zug':78,'schwyz':100,'uri':100,'obwalden':73,'nidwalden':80,'glarus':100,'fribourg':145,'solothurn':125,'schaffhausen':100,'appenzell':115,'stgallen':115,'graubunden':100,'aargau':112,'thurgau':119,'ticino':90,'vaud':155,'valais':135,'neuchatel':130,'geneve':141,'jura':120};
    if (plzStr.startsWith('3')) return OTHER_TAX['bern'];
    if (plzStr.startsWith('4')) return OTHER_TAX['basel'];
    if (plzStr.startsWith('6')) return OTHER_TAX['luzern'];
    if (plzStr.startsWith('63') || plzStr.startsWith('64')) return OTHER_TAX['zug'];
    if (plzStr.startsWith('12')) return OTHER_TAX['geneve'];
    if (plzStr.startsWith('10') || plzStr.startsWith('11')) return OTHER_TAX['vaud'];
    if (plzStr.startsWith('7')) return OTHER_TAX['graubunden'];
    if (plzStr.startsWith('9')) return OTHER_TAX['stgallen'];
    return null;
  }
  const taxRate = lookupTax(communeLower, plz);
    return res.status(405).json({ error: 'Method not allowed' });
  }
  ...
  const TAX_OTHER = {
    'bern':116,'berne':116,'köniz':116,'thun':108,'interlaken':104,'langenthal':112,
    'basel':96,'bale':96,'riehen':92,'arlesheim':68,'binningen':68,'bottmingen':68,
    'genf':45,'geneve':45,'geneva':45,
    'lausanne':79,'pully':70,'lutry':68,'morges':74,'nyon':62,'renens':86,
    'luzern':107,'lucerne':107,'kriens':107,'emmen':110,'horw':83,'meggen':64,
    ...
    'solothurn':110,'olten':110,'zofingen':106,'grenchen':115,'chur':117,
  };

  function lookupTax(comm, plzStr) {
    const norm = comm.replace(/\./g,'').replace(/-/g,' ').replace(/\s+/g,' ').trim();
    if (String(plzStr).startsWith('8')) {
      if (TAX_ZH[norm] !== undefined) return TAX_ZH[norm];
      const short = norm.replace(/ a[adb]$/, '').replace(/ im .*$/, '').replace(/ am .*$/, '').trim();
      if (short !== norm && TAX_ZH[short] !== undefined) return TAX_ZH[short];
      const first = norm.split(' ')[0];
      if (first.length > 4) {
        const match = Object.entries(TAX_ZH).find(([k]) => k === first || k.startsWith(first + ' '));
        if (match) return match[1];
      }
    }
    if (TAX_OTHER[norm] !== undefined) return TAX_OTHER[norm];
    for (const [k,v] of Object.entries(TAX_OTHER)) {
      if (norm.includes(k)) return v;
    }
    return null;
  }

  const ZURICH_QUARTERS = ['wollishofen','leimbach','enge','langstrasse','hard','gewerbeschule','hochschulen','sihlfeld','friesenberg','albisrieden','altstetten','wipkingen','höngg','oberstrass','unterstrass','seebach','oerlikon','schwamendingen','witikon','riesbach','fluntern','hottingen','hirslanden','aussersihl','escher wyss','rathaus'];
  const isZurichQuarter = ZURICH_QUARTERS.some(q => communeLower.includes(q));
  let steuerfuss = isZurichQuarter ? 119 : lookupTax(communeLower, String(plz||''));
...
- Zimmeranzahl ist EXAKT: ${rooms} Zimmer.
...
${steuerfuss?`
${s.taxSection}
CHF 120'000 Jahreseinkommen, Steuerfuss ${steuerfuss}% vs. Zürich-Stadt (119%).`:''}

${s.checklistSection}
8–10 spezifische Punkte.
