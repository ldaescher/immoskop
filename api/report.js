export default async function handler(req, res) {
  ...
  const { address, rooms, area, price, type, year, floor, outdoor, condition, insertText, parsedInsert, propertyKind, reportDate, extraInfo, lang,
    expectedFromData, deltaFromData, priceSourceFromData, autobahnName, autobahnRichtungen, autobahnDist, autobahnFahrzeit,
    marktlageFromData, lwzFromData } = req.body;
...
` : ''}${s.priceSection}
${(() => {
  // Verdict aus data.js übernehmen und als explizite Anweisung formulieren
  const d = deltaFromData !== undefined && deltaFromData !== null ? deltaFromData : delta;
  const ml = marktlageFromData || 'normal';
  const lwzVal = lwzFromData !== undefined ? lwzFromData : (req.body.lwz || 1.08);
  let verdictLabel, verdictInstruction;
  if (d >= 20)       { verdictLabel = 'stark überteuert';    verdictInstruction = `Das Angebot ist STARK ÜBERTEUERT (+${d}%). Begründe klar warum. Widerspreche dieser Einschätzung nicht.`; }
  else if (d >= 10)  { verdictLabel = 'spürbar überteuert';  verdictInstruction = `Das Angebot ist SPÜRBAR ÜBERTEUERT (+${d}%). Erkläre die Preisabweichung kritisch. Widerspreche dieser Einschätzung nicht.`; }
  else if (d >= 3)   { verdictLabel = 'leicht überteuert';   verdictInstruction = `Das Angebot ist LEICHT ÜBERTEUERT (+${d}%). Erwähne dies sachlich. Widerspreche dieser Einschätzung nicht.`; }
  else if (d >= -3)  { verdictLabel = 'marktkonform';        verdictInstruction = `Das Angebot ist ZUM MARKTPREIS (+${d}%). Bewerte es als fair/marktkonform. Widerspreche dieser Einschätzung nicht.`; }
  else if (d >= -10) { verdictLabel = 'leicht günstig';      verdictInstruction = `Das Angebot ist LEICHT GÜNSTIG (${d}%). Erwähne dies positiv. Widerspreche dieser Einschätzung nicht.`; }
  else if (d >= -20) { verdictLabel = 'günstig';             verdictInstruction = `Das Angebot ist GÜNSTIG (${d}%). Hebe dies als Vorteil hervor. Widerspreche dieser Einschätzung nicht.`; }
  else               { verdictLabel = 'sehr günstig';        verdictInstruction = `Das Angebot ist SEHR GÜNSTIG (${d}%). Hebe dies deutlich hervor. Widerspreche dieser Einschätzung nicht.`; }
  return `FRONTEND-VERDICT: ${verdictLabel} (${d > 0 ? '+' : ''}${d}% vom Referenzpreis CHF ${(expectedFromData || expected).toLocaleString('de-CH')}).
PFLICHT: ${verdictInstruction}
Marktlage: LWZ ${lwzVal}% → ${ml === 'extrem_angespannt' ? 'extrem angespannt' : ml === 'angespannt' ? 'angespannt' : ml === 'entspannt' ? 'entspannt' : 'normal'} — berücksichtige dies in der Preisbewertung.`;
})()}
