export default async function handler(req, res) {
  ...
  const { address, rooms, area, price, type, year, floor, outdoor, condition, insertText, parsedInsert, propertyKind, reportDate, extraInfo } = req.body;
  ...
  // Haus vs. Wohnung Faktor (Häuser typisch 12-18% teurer pro m²)
  const PROPERTY_F = {'haus':1.15,'wohnung':1.0};
  const propertyFactor = PROPERTY_F[propertyKind] || 1.0;
  ...
  if (placesResult.status === 'fulfilled' && placesResult.value) {
    try {
      ...
      // Noise estimate from nearest road (fallback)
      if (!noiseDay && oevDist) {
        noiseDay = 48; // residential default
        noiseSource = 'Schatzung (Wohngebiet)';
      }
  ...
  // ── 8. STEUERFUSS ───────────────────────────────────────────────
  // Zürich city quarters/districts - all have Zürich steuerfuss 119%
  const ZURICH_QUARTERS = ['wollishofen','leimbach','enge','langstrasse','hard','gewerbeschule','hochschulen','lindenhügel','hochfeld','werd','sihlfeld','friesenberg','albisrieden','altstetten','wipkingen','höngg','oberstrass','unterstrass','seebach','affoltern','oerlikon','saatlen','schwamendingen','hirzenbach','witikon','mühlebach','weinegg','riesbach','fluntern','hottingen','hirslanden','wiedikonn','aussersihl','city','rathaus','lindenhügel','escher wyss','gewerbeschule'];
  const TAXES = {
    'kilchberg':72,'küsnacht':73,'zollikon':78,'herrliberg':75,'thalwil':80,'rüschlikon':73,
    'adliswil':84,'langnau am albis':90,'oberrieden':74,'horgen':86,
    ...
    'luzern':107,'lucerne':107,'kriens':107,'emmen':110,
    'lugano':72,'locarno':78,'bellinzona':82,'mendrisio':80,
    'sion':122,'sitten':122,'martigny':125,'monthey':118,
    'freiburg':93,'fribourg':93,'murten':84,'bulle':95,
    'biel':107,'bienne':107,'nidau':100,
    'neuchatel':115,'neuenburg':115,'la chaux-de-fonds':118,
    'st. gallen':116,'st.gallen':116,'gossau':111,'rapperswil':104,
    'zug':82,'baar':82,'cham':84,'steinhausen':78,'hunenberg':72,
    'aarau':106,'baden':100,'wettingen':103,'brugg':104,'lenzburg':100,
    'laax':98,'davos':95,'st. moritz':85,'klosters':90,'arosa':90,
    'verbier':90,'zermatt':85,'crans':95,'saas-fee':90,
    'schaffhausen':113,'neuhausen':110,'frauenfeld':106,'kreuzlingen':110,
    'solothurn':110,'olten':110,'zofingen':106,'grenchen':115,
    'thun':108,'spiez':108,'interlaken':104,'langenthal':112,
    'chur':117,'arlesheim':68,'binningen':68,'bottmingen':68,
  };
  // Check if address is in a Zürich city quarter
...
Heutiges Datum: ${today}. Beurteile Verfügbarkeitsdaten relativ zu diesem Datum.

WICHTIGE REGELN:
- Erfinde KEINE Angaben die nicht in den Daten oder im Inseratstext stehen.
- Die Zimmeranzahl ist EXAKT: ${rooms} Zimmer. Nenne IMMER genau diese Zahl. Wenn der Inseratstext eine andere Zahl nennt, zitiere beide — aber erfinde keine dritte Zahl.
- Wenn der Inseratstext eine Angabe macht, zitiere genau was dort steht.
- Wenn du Widersprüche bemerkst, zitiere beide Quellen wörtlich bevor du kommentierst.
- "Nicht verfügbar" ist besser als eine erfundene Angabe.
- Schreibe NIEMALS "X Zimmer" ausser die Zahl ist ${rooms}.

INSERAT:
Adresse: ${label}${geoAccuracy === 'gemeinde' ? ' (NUR GEMEINDE BEKANNT — keine strassengenaue Analyse möglich, Resultate sind Gemeinde-Durchschnitt)' : ''}
Typ: ${isKauf?(propertyKind==='haus'?'Kaufhaus/EFH':'Kaufwohnung'):(propertyKind==='haus'?'Miethaus/EFH':'Mietwohnung')} | ${rooms} Zimmer | ${area} m² | Etage ${floor} | Baujahr ${year}
...
${insertText ? `INSERATSTEXT (Original):
${insertText.substring(0, 2000)}

INSERAT-ANALYSE:
${inseratZusammenfassung || 'nicht verfügbar'}
${befristet ? '*** ACHTUNG: BEFRISTETER MIETVERTRAG ***\n' + (befristetDetails || '') : ''}
${besonderheiten.length ? 'Besonderheiten: ' + besonderheiten.join(', ') : ''}
${marketingFlags.length ? 'Marketing-Begriffe zum Hinterfragen: ' + marketingFlags.join(', ') : ''}
...
${befristet ? `## !! ACHTUNG: BEFRISTETER MIETVERTRAG !!
WICHTIG: Laut Inseratstext ist dieser Mietvertrag befristet${befristetDetails ? ': ' + befristetDetails : ''}. Erkläre die Konsequenzen für den Mieter (kein Kündigungsschutz, erzwungener Auszug, Schwierigkeiten bei Verlängerung).
...
## Inserat-Analyse
${insertText ? `Analysiere den Inseratstext kritisch: Was verspricht der Vermieter/Verkäufer? Was davon ist mit unseren Daten überprüfbar? Entlarve Marketing-Begriffe (${marketingFlags.join(', ') || 'keine spezifischen gefunden'}).` : 'Kein Inseratstext vorhanden.'}
