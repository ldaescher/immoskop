export default async function handler(req, res) {
  ...
  try {
    ...
    if (streetName) {
      ...
      // Fix: Bei typ='gemeinde' ist gemeinde_slug immer null (Gemeinde-Rows verweisen nicht auf sich selbst).
      // In diesem Fall den slug des Gemeinde-Rows direkt verwenden – funktioniert für alle Gemeinden
      // mit nur einer PLZ (z.B. Kilchberg, Thalwil, etc.) die keinen Locality-Eintrag haben.
  ...
  // Autobahnausfahrten via Overpass
  const autobahnPromise = (async () => {
    try {
      // Junction-Radius bewusst klein (8km): In Grossstädten wie Zürich gibt es viele Junctions.
      // Mit Limit 100 und kleinem Radius stellen wir sicher dass die nächste Ausfahrt dabei ist,
      // ohne dass das Limit von weiter entfernten Nodes aufgebraucht wird.
      // out geom ist nötig damit Nodes ihre lat/lon bekommen.
      const q = `[out:json][timeout:15];(
        node[highway=motorway_junction](around:8000,${lat},${lon});
        way[highway=motorway](around:20000,${lat},${lon});
        way[highway=motorway_link](around:3000,${lat},${lon});
      );out geom 100;`;
  ...
  // Quelle: Tagesanzeiger/BFS, Leerwohnungszählung 2024 (454 Gemeinden)
  // Update jährlich im September nach BFS-Publikation!
  // Priorität: Gemeinde-spezifisch → Kantons-Fallback → CH-Schnitt
  const LWZ_GEMEINDE = {
    'aarau': 0.38,
    'aarberg': 1.44,
    'aarburg': 4.41,
    ...
    'commugny': 8.48,
    'confignon': 0.38,
    'corcelles-le-jorat': 3.5,
    'corseaux': 2.21,
    'corsier-sur-vevey': 0.66,
    'courchapoix': 5.88,
    'courchavon': 3.11,
    'courgenay': 3.1,
    'courtedoux': 3.75,
    ...
    'crésuz': 3.3,
    'curtilles': 3.82,
    'damphreux-lugnez': 3.47,
    'davos': 0.1,
    'delémont': 4.56,
    'diessenhofen': 1.47,
    'dietikon': 0.55,
    'disentis/mustér': 0.21,
    'domat/ems': 0.53,
    'domleschg': 1.54,
    'dulliken': 4.11,
    'dörflingen': 3.43,
    ...
    'kloten': 0.36,
    'konstanz': 0.0,
    'kradolf-schönenberg': 1.14,
    'kreuzlingen': 1.29,
    'kriens': 0.51,
    'köniz': 0.28,
    'küsnacht (zh)': 1.34,
    'küssnacht (sz)': 0.32,
    'la chaux-de-fonds': 3.57,
    'la sagne': 4.69,
    'la tour-de-peilz': 0.56,
    'laax': 0.82,
    'lachen': 0.24,
    'lancy': 0.16,
    'landquart': 0.11,
    ...
    'meyrin': 0.16,
    'minusio': 1.1,
    'misery-courtion': 3.49,
    'mont-sur-rolle': 3.7,
    'monthey': 0.24,
    'montreux': 1.13,
    'morbio inferiore': 6.32,
    'morges': 0.32,
    'moudon': 0.3,
    'moutier': 6.01,
    'mumpf': 2.86,
    ...
    'petit-val': 3.57,
    'pfäffikon': 0.13,
    'plan-les-ouates': 0.21,
    'plasselb': 7.06,
    'porrentruy': 4.09,
    'port': 4.16,
    'poschiavo': 2.84,
    'pratteln': 0.78,
    'pregny-chambésy': 0.55,
    'prilly': 0.58,
    'prévonloup': 4.85,
    'pully': 0.33,
    'rapperswil-jona': 1.98,
    'regensdorf': 0.26,
    'reiden': 3.49,
    'reinach (ag)': 1.32,
    'reinach (bl)': 0.92,
    ...
    'roches (be)': 5.38,
    'rolle': 2.03,
    'romanel-sur-morges': 4.67,
    'romanshorn': 3.07,
    'romont (be)': 4.0,
    'rongellen': 4.0,
    'root': 1.02,
    'rorschach': 2.15,
    'rorschacherberg': 3.11,
    'rothenthurm': 0.41,
    'roveredo (gr)': 5.24,
    'rumisberg': 2.91,
    'rüschlikon': 0.64,
    ...
    'volketswil': 0.44,
    'vucherens': 4.48,
    'vufflens-le-château': 2.87,
    'vugelles-la mothe': 10.29,
    'vully-les-lacs': 3.56,
    'walchwil': 0.9,
    'wald (zh)': 2.02,
    'wallisellen': 2.98,
    'wangen (sz)': 0.68,
    'wangen bei olten': 2.95,
    'weinfelden': 1.41,
    'wetzikon (zh)': 0.42,
    'wil (sg)': 0.92,
    'wil (zh)': 0.57,
  ...
  // Kantons-Fallback (wenn Gemeinde nicht in LWZ_GEMEINDE)
  const LWZ_KANTON = {
  ...
  // Lookup: Gemeinde → Kanton → CH-Schnitt
  const communeNameLower = (priceData?.name || label || '').toLowerCase()
    .replace(/\s*\(.*?\)/g, '').trim(); // Klammern entfernen z.B. "Aarau" aus "Aarau (AG)"
  const lwzGemeinde = LWZ_GEMEINDE[communeNameLower] ?? null;
  const lwzKanton = priceData?.kanton || null;
  const lwz = lwzGemeinde ?? (lwzKanton ? (LWZ_KANTON[lwzKanton] ?? 1.08) : 1.08);
  const lwzSource = lwzGemeinde !== null ? 'gemeinde' : lwzKanton ? 'kanton' : 'ch';
  // Marktlage basierend auf LWZ
  const marktlage = lwz < 0.5 ? 'extrem_angespannt' : lwz < 1.0 ? 'angespannt' : lwz < 2.0 ? 'normal' : 'entspannt';
  console.log('LWZ:', lwz + '%', '(' + communeNameLower + ' → ' + lwzSource + ') → Marktlage:', marktlage);
  ...
  try {
    ...
    if (junctions.length > 0) {
      ...
      if (nearest) {
        ...
        // Ref-Reihenfolge: Junction-Node motorway:ref → Junction ref → Way-Refs → null
